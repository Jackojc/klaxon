...
namespace klx {
	...
	// i.e. KLX_STR(__LINE__) => "42" as opposed to "__LINE__"
	#define KLX_STR_IMPL_(x) #x
	#define KLX_STR(x) KLX_STR_IMPL_(x)

	// Concatenate macro defs.
	// i.e. KLX_CAT(__FILE__, __LINE__) => "foo.c10" as opposed to "__FILE____LINE__"
	#define KLX_CAT_IMPL_(x, y) x##y
	#define KLX_CAT(x, y) KLX_CAT_IMPL_(x, y)

	// Create a uniquely named variable for use in a macro.
	#define KLX_VAR(x) KLX_CAT(var_, KLX_CAT(x, KLX_CAT(__LINE__, _)))

	// Evaluate expressions at beginning and ending of a scope.
	#define KLX_SCOPE(open, close) \
		for ( \
			br::index_t KLX_VAR(i) = ((open), 0); \
			!KLX_VAR(i); \
			(KLX_VAR(i)++), (close) \
		)

	// Evaluate expression at end of scope.
	#define KLX_DEFER(close) \
		for ( \
			br::index_t KLX_VAR(i) = 0; \
			!KLX_VAR(i); \
			(KLX_VAR(i)++), (close) \
		)
	...
	#define KLX_TRACE "[" __FILE__ ":" KLX_STR(__LINE__) "] "


	// Detect platform.
	#if defined(_WIN64) || defined(_WIN32) || defined(__WINDOWS__)
		#define KLX_PLATFORM_WINDOWS
	#elif defined(__linux) || defined(linux)
		#define KLX_PLATFORM_LINUX
	#elif defined(__DragonFly__) || defined(__FreeBSD__) || defined(__NETBSD__) || defined(__OpenBSD__)
		#define KLX_PLATFORM_BSD
	#elif defined(__APPLE__) || defined(macintosh) || defined(__MACH__)
		#define KLX_PLATFORM_OSX
	#elif defined(__unix) || defined(unix)
		#define KLX_PLATFORM_UNIX
	#else
		#define KLX_PLATFORM_UNKNOWN
	#endif


	// Detect compiler.
	#if defined(__GNUC__)
		#define KLX_COMPILER_GCC
	#elif defined(__INTEL_COMPILER)
		#define KLX_COMPILER_INTEL
	#elif defined(__clang__)
		#define KLX_COMPILER_CLANG
	#elif defined(_MSC_VER)
		#define KLX_COMPILER_MSVC
	#else
		#define KLX_COMPILER_UNKNOWN
	#endif
